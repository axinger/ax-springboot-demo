-- 插入用户
INSERT INTO sys_users (username, password, email, real_name, department, position) VALUES
                                                                                       ('admin', '$2a$10$N.zmdaEie6zW.OC3u/.eUu0bT0QmIjX8XwXwLQd8fZ9xK9XxK9XxK', 'admin@example.com', '系统管理员', 'IT部', '系统管理员'),
                                                                                       ('sales_director', '$2a$10$N.zmdaEie6zW.OC3u/.eUu0bT0QmIjX8XwXwLQd8fZ9xK9XxK9XxK', 'sales@example.com', '销售总监', '销售部', '销售总监'),
                                                                                       ('order_manager', '$2a$10$N.zmdaEie6zW.OC3u/.eUu0bT0QmIjX8XwXwLQd8fZ9xK9XxK9XxK', 'order@example.com', '订单经理', '订单部', '订单经理'),
                                                                                       ('warehouse_manager', '$2a$10$N.zmdaEie6zW.OC3u/.eUu0bT0QmIjX8XwXwLQd8fZ9xK9XxK9XxK', 'warehouse@example.com', '仓库经理', '仓储部', '仓库经理'),
                                                                                       ('employee1', '$2a$10$N.zmdaEie6zW.OC3u/.eUu0bT0bT0QmIjX8XwXwLQd8fZ9xK9XxK9XxK', 'emp1@example.com', '员工1', '销售部', '销售专员');
...
-- 插入API
-- 订单相关API
INSERT INTO sys_apis (path, method, service_name, description, category) VALUES
                                                                             ('/api/orders', 'GET', 'order-service', '查询订单列表', '订单管理'),
...
-- 销售相关API
INSERT INTO sys_apis (path, method, service_name, description, category) VALUES
                                                                             ('/api/sales/reports', 'GET', 'sales-service', '销售报表', '销售管理'),
...
-- 仓库相关API
INSERT INTO sys_apis (path, method, service_name, description, category) VALUES
                                                                             ('/api/warehouses', 'GET', 'warehouse-service', '查询仓库列表', '仓库管理'),
...
-- 系统相关API
INSERT INTO sys_apis (path, method, service_name, description, category) VALUES
                                                                             ('/api/users', 'GET', 'system-service', '查询用户列表', '系统管理'),
...
-- 插入权限
-- 系统管理权限
INSERT INTO sys_permissions (name, code, description, category) VALUES
                                                                    ('用户管理', 'user:manage', '允许管理用户', '系统权限'),
...
-- 销售管理权限
INSERT INTO sys_permissions (name, code, description, category) VALUES
                                                                    ('销售数据查看', 'sales:data:read', '允许查看销售数据', '销售权限'),
...
-- 仓库管理权限
INSERT INTO sys_permissions (name, code, description, category) VALUES
                                                                    ('仓库查看', 'warehouse:read', '允许查看仓库信息', '仓库权限'),
...
WHERE p.code = 'order:read' AND a.path IN ('/api/orders', '/api/orders/{id}') AND a.method = 'GET';

INSERT INTO sys_permission_apis (permission_id, api_id)
SELECT p.id, a.id
FROM sys_permissions p, sys_apis a
WHERE p.code = 'order:create' AND a.path = '/api/orders' AND a.method = 'POST';

INSERT INTO sys_permission_apis (permission_id, api_id)
SELECT p.id, a.id
FROM sys_permissions p, sys_apis a
WHERE p.code = 'order:update' AND a.path = '/api/orders/{id}' AND a.method = 'PUT';

INSERT INTO sys_permission_apis (permission_id, api_id)
SELECT p.id, a.id
FROM sys_permissions p, sys_apis a
WHERE p.code = 'order:delete' AND a.path = '/api/orders/{id}' AND a.method = 'DELETE';

INSERT INTO sys_permission_apis (permission_id, api_id)
SELECT p.id, a.id
FROM sys_permissions p, sys_apis a
WHERE p.code = 'order:approve' AND a.path = '/api/orders/approve' AND a.method = 'POST';

-- 销售权限关联
INSERT INTO sys_permission_apis (permission_id, api_id)
SELECT p.id, a.id
FROM sys_permissions p, sys_apis a
WHERE p.code = 'sales:data:read' AND a.path = '/api/sales/data' AND a.method = 'GET';

INSERT INTO sys_permission_apis (permission_id, api_id)
SELECT p.id, a.id
FROM sys_permissions p, sys_apis a
WHERE p.code = 'sales:report:read' AND a.path = '/api/sales/reports' AND a.method = 'GET';

INSERT INTO sys_permission_apis (permission_id, api_id)
SELECT p.id, a.id
FROM sys_permissions p, sys_apis a
WHERE p.code = 'sales:target:set' AND a.path = '/api/sales/targets' AND a.method = 'POST';

INSERT INTO sys_permission_apis (permission_id, api_id)
SELECT p.id, a.id
FROM sys_permissions p, sys_apis a
WHERE p.code = 'customer:info:read' AND a.path = '/api/customers' AND a.method = 'GET';

INSERT INTO sys_permission_apis (permission_id, api_id)
SELECT p.id, a.id
FROM sys_permissions p, sys_apis a
WHERE p.code = 'customer:assign' AND a.path = '/api/customers/assign' AND a.method = 'POST';

-- 仓库权限关联
INSERT INTO sys_permission_apis (permission_id, api_id)
SELECT p.id, a.id
FROM sys_permissions p, sys_apis a
WHERE p.code = 'warehouse:read' AND a.path IN ('/api/warehouses', '/api/warehouses/{id}') AND a.method = 'GET';

INSERT INTO sys_permission_apis (permission_id, api_id)
SELECT p.id, a.id
FROM sys_permissions p, sys_apis a
WHERE p.code = 'stock:read' AND a.path = '/api/stocks' AND a.method = 'GET';
...
SELECT u.id, r.id
FROM sys_users u, sys_roles r
WHERE u.username = 'sales_director' AND r.code = 'ROLE_SALES_DIRECTOR';

-- 订单经理
INSERT INTO sys_user_roles (user_id, role_id)
SELECT u.id, r.id
FROM sys_users u, sys_roles r
WHERE u.username = 'order_manager' AND r.code = 'ROLE_ORDER_MANAGER';

-- 仓库经理
INSERT INTO sys_user_roles (user_id, role_id)
SELECT u.id, r.id
FROM sys_users u, sys_roles r
WHERE u.username = 'warehouse_manager' AND r.code = 'ROLE_WAREHOUSE_MANAGER';

-- 员工1（销售专员）
INSERT INTO sys_user_roles (user_id, role_id)
SELECT u.id, r.id
FROM sys_users u, sys_roles r
WHERE u.username = 'employee1' AND r.code = 'ROLE_SALESPERSON';
